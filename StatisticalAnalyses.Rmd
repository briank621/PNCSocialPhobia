...
```{r}
spSex = ifelse(covars.fmri[status=='SP']$Sex=="F", 0, 1)
controlSex = ifelse(covars.fmri[status=='Control']$Sex == "F", 0, 1)
perm.t.test(spSex, controlSex, R = 10000) # two sample t-test (SP, Control)
```
### Age

```{r}
spAge = as.numeric(covars.fmri[status=='SP']$age_at_cnb)
controlAge = as.numeric(covars.fmri[status=='Control']$age_at_cnb)
perm.t.test(spSex, controlSex, R = 10000) # two sample t-test (SP, Control)
```
...
numSP = length(covars.fmri$status[covars.fmri$status=="SP"])
