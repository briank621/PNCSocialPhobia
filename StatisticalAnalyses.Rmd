...
A.all.norm.sub = readRDS(file.path(savedir,'CCAAL.A.all.norm.sub.rds'))
```
...
```{r}
saveRDS(fcResults, file=file.path(savedir, 'CCAAL.fcResults.rds'), compress='xz')
fcResults = readRDS(file.path(savedir, 'CCAAL.fcResults.rds'))
```
...
for(i in 1:length(fcResults)){
    ...
    if(numEdges != 0){
        for(j in 1:numEdges){
            confInt[j] = extractConfidence(fcResults[[i]]$significantPValues[j], A.pos.norm.sub, controlGroup, experimentalGroup)
        }
    ...
    if(numEdges != 0){
        for(j in 1:numEdges){
            confInt[j] = extractConfidence(fcResults[[i]]$uncorrected[j], A.pos.norm.sub, controlGroup, experimentalGroup)
        }
...
#saveRDS(seedResults, file=file.path(savedir, 'GAALMotion.seedResults.rds'), compress='xz')
#A.all.norm.sub = readRDS(file.path(savedir,'CCAAL.all.norm.sub.rds'))
#A.pos.norm.sub = readRDS(file.path(savedir,'CCAAL.A.pos.norm.sub.rds'))
#saveRDS(fcResults, file=file.path(savedir, 'GAALMotion.fcResults.rds'), compress='xz')


#saveRDS(seedResults, file=file.path(savedir, 'GAAL.seedResults.rds'), compress='xz')
#seedResults = readRDS(file.path(savedir, 'CCAAL.seedResults.rds'))
fcResults = readRDS(file.path(savedir, 'CCAAL.fcResults.rds'))
```
