...
g.all.sub = readRDS(file.path(savedir,'g.all.sub.rds'))
...
AUCResults = cbind(AUCResults, covars.fmri[, .(SUBJID, status, smry_soc)])
...
```{r}
dt.g.all.sub.graph$max.frac.comp = dt.g.all.sub.graph[,.(max.comp)] / 264
```
...
for(metric in metrics){
    result = computeAUC(dt.g.all.sub.graph, covars.fmri, thresholds, metric, "SUBJID", small.dt, dt.g.all.sub.graph)
...
saveRDS(small.dt, file=file.path(savedir, 'small.dt.rds'), compress='xz')
```

```{r}
## Loading precomputed results
small.dt = readRDS(file.path(savedir,'small.dt.rds'))
```
...
for(metric in metrics){
    result = computeAUC(small.dt, covars.fmri, thresholds, metric, "SUBJID", small.dt, dt.g.all.sub.graph)
    AUCResults = merge(AUCResults, result)
}
```
...
globalPValues = data.table(p.value=numeric(), metric=character())[1:length(metrics)]
for(index in 1:length(metrics)){
    metric = metrics[[index]]
    spList = AUCResults[status=="SP"][[metric]]
    controlList = AUCResults[status=="Control"][[metric]]
    ttest = perm.t.test(spList, controlList, R = 10000) # two sample t-test (SP, Control)
    globalPValues[index] = list(ttest$perm.p.value, metric)
}
```
...
eNodalResult = computeRegionalAUC(dt.g.all.sub.vertex, covars.fmri, thresholds, "E.nodal")
localResult = computeRegionalAUC(dt.g.all.sub.vertex, covars.fmri, thresholds, "E.local")
degreeResult = computeRegionalAUC(dt.g.all.sub.vertex, covars.fmri, thresholds, "degree")
```

We can also check nodal statistics individually. This 

```{r}
source("ComputeSingleRegionAUC.R")
singleAUCResults = data.table()
singleAUCResults = cbind(AUCResults, covars.fmri[, .(SUBJID, status, smry_soc)])
...
regions = c("Cingulum_Post.L1", "Cingulum_Post.L2", "Cingulum_Post.L3", "Cingulum_Post.L4", "Putamen.R1", "Putamen.R2", "Putamen.R3")
for(region in regions){
    result = computeSingleRegionAUC(dt.g.all.sub.vertex, region, covars.fmri, thresholds, "E.local");
    singleAUCResults = merge(singleAUCResults, result)
}
```
