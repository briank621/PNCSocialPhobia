...
regionName = "SOG.R"
regions = c("SOG.L", "SOG.R", "PCL.R")
metrics = c("E.nodal", "E.nodal", "E.local")
for(i in 1:length(regions)){
    result = computeSingleRegionAUC(dt.g.all.sub.vertex, regions[i], covars.fmri, thresholds, metrics[i]);
...
```{r}
# go through all edges get a count

dt = data.table(status=covars.fmri$status, Lp=AUCResults$Lp, Cp=AUCResults$Cp, E.local=AUCResults$E.local)
# dt[,Lp.norm:= AUCResults$Lp.norm]

count = 1
seen = array(0, dim=c(116,116))

for(i in 1:4){
    uncorrected = fcResults[[i]]$uncorrected
    for(j in 1:NROW(uncorrected)){
        r1i = uncorrected[j]$region1Index
        r2i = uncorrected[j]$region2Index
        if(seen[r1i, r2i] == 1){ # ignore duplicate edges
            next
        }
        seen[r1i, r2i] = 1
        pairName = paste('Pair', count, sep='')
        count = count + 1
        fcValue = A.pos.norm.sub[[1]][r1i, r2i, ]
        dt[,(pairName):= fcValue]
    }
}

# go through all topological measures

# Implicated vertices are eNodal: 49, 50 and local: 70
# This is using the singleAUCResults code from abova

# dt = cbind(dt, singleAUCResults[, -4:-1])

# Write results to CSV
costdir = paste(datadir, "/cost_sensitive/", sep="")

# Need to sort by statuses and then print them out
filePath = paste(costdir, "ControlSPADHDT.csv", sep="")
write.csv(dt[status == "SPADHD" | status == "Control"], filePath)

filePath = paste(costdir, "ControlSPT.csv", sep="")
write.csv(dt[status == "SP" | status == "Control"], filePath)

filePath = paste(costdir, "SPSPADHDT.csv", sep="")
write.csv(dt[status == "SP" | status == "SPADHD"], filePath)

# TODO: Try and get the edges the same as the other one
```
