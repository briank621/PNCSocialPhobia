...
covars.fmri$status = ifelse(covars.fmri$smry >= 3, "SP", "Control")
...
saveRDS(A.all.norm.sub, file=file.path(savedir, 'A.all.norm.sub.rds'), compress='xz')
```
...
saveRDS(g.all.sub, file=file.path(savedir, 'g.all.sub.rds'), compress='xz')
saveRDS(g.all.group, file=file.path(savedir, 'g.all.group.rds'), compress='xz')
```
