...
```{r}
datadir = '/Users/brian/Documents/classes/quantitativepsych/PNCSocialPhobia'
...
for (subjectID in covars.fmri$SUBJID){
    subjectPaths[count] = paste(datadir, '/datafiles/ROICorrelation_FisherZ_sub_', subjectID, '.txt', sep='')
    count = count + 1
}
...
saveRDS(A.all.norm.sub, file=file.path(savedir, 'ADHD.A.all.norm.sub.rds'), compress='xz')
```
...
g.all = createGraphs(A.all.bin.sub, A.all.bin.mean, thresholds, covars)
...
saveRDS(g.all.sub, file=file.path(savedir, 'ADHD.g.all.sub.rds'), compress='xz')
saveRDS(g.all.group, file=file.path(savedir, 'ADHD.g.all.group.rds'), compress='xz')
```
