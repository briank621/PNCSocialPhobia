...
for (subjectID in covars.fmri$SUBJID){
    subjectPaths[count] = paste(datadir, '/aal_datafiles/ROICorrelation_FisherZ_sub_', subjectID, '.txt', sep='')
    count = count + 1
}
...
saveRDS(A.all.norm.sub, file=file.path(savedir, 'AAL.A.all.norm.sub.rds'), compress='xz')
```
...
saveRDS(g.all.sub, file=file.path(savedir, 'AAL.g.all.sub.rds'), compress='xz')
